...
describe('use default branding', () => {
  ...
  it('document title should be test-Soph-IA if set title', () => {
    renderHook(() => useDocumentTitle('test'))
    expect(document.title).toBe('test - Soph-IA')
  })

  it('document title should be Soph-IA if not set title', () => {
    renderHook(() => useDocumentTitle(''))
    expect(document.title).toBe('Soph-IA')
  })
})
